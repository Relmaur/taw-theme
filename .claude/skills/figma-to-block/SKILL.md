...
**A named breakpoint-variant sibling is a second independent source of truth, not a derived reflow.** If `get_metadata` shows a section has sibling frames named for specific breakpoints (e.g. "Desktop (1440px)" / "Mobile (375px)" under the same section group), don't assume the narrower one is just the wide one reflowed — a designer routinely re-crops images, changes stacking order, or resizes elements independently per breakpoint. Pull a separate `get_design_context` for each named breakpoint frame you intend to actually implement (at minimum desktop; mobile too if the request needs a working small-viewport layout), and implement each breakpoint's specifics from its own frame's data, not by guessing how the desktop frame's values should scale down.
...
- **Colors/sizes from the design that aren't already project design tokens** (check `resources/scss/app.scss`/`resources/css/app.css` for a Tailwind `@theme` block, and `_fonts.scss`/`resources/fonts/` for existing type) should be applied as Tailwind arbitrary values (`bg-[#12212b]`, `text-[64px]`) rather than invented utility/theme names — don't silently add global `@theme` tokens for a one-off section. Mention to the user that promoting a color/font to a shared token is worth doing if the same design system recurs across sections. **If this block is being built via `build-page`, its Step 2 already extracted the file's full token set into that `@theme` block before any section work started** — check there first and reference the existing token (`bg-primary-600`, etc.) instead of an arbitrary value for anything that traces back to a named Figma variable/style; arbitrary values are for genuine one-offs the token set doesn't cover, not the default for every value.
- **A shared fluid/`clamp()`-based utility class does not necessarily reproduce a section's own literal spacing/sizing token, even when it "looks like padding."** If the Figma section frame's own `get_design_context` output shows a literal value (e.g. a named spacing variable like `gap-80` resolving to a fixed `80px`, not something computed), that literal value is the source of truth for *this* section — don't assume a shared fluid class (e.g. a project-wide `.section-y`/`.section-shell` built from `clamp(...)`) happens to resolve to the same number at this viewport. Map the literal Figma token to the project's own equivalent static utility instead (Tailwind's own scale is `n × 0.25rem`, so a `gap-80`/`80px` token ⇔ `p-20`/`py-20`), and verify by measuring the actual rendered result (see Step 7) rather than assuming the fluid class matches. A shared fluid class is only correct here if it was itself deliberately reverse-engineered from this exact section's spec.
...
- **Cross-check Tailwind values by hand** against the Figma spec if you can't compile the actual stylesheet (this project's Tailwind v4 only builds through Vite, no standalone CLI) — Tailwind's default spacing scale is `n × 0.25rem`, so e.g. Figma's `px-[80px]` ⇔ project's `px-20` (20 × 4px = 80px). Do this arithmetic rather than asserting a match you haven't checked. **This hand arithmetic only works for a fixed-scale utility.** A shared fluid/`clamp()`-based class (see Step 5) has no single px value to check by hand — its resolved size depends on the actual viewport width, so hand-checking it means resolving the `clamp()` formula algebraically at the target width, or (once a browser check is underway anyway, see below) reading the real computed value directly rather than guessing.
- **If the developer has already agreed to a browser check** (never trigger one on your own — same standing "ask first" policy the bullet below and `visual-check` itself follow), prefer an iterative measure-and-adjust loop over a single end-of-work pixel comparison for anything spacing/position-sensitive: build the section, measure its actual computed values (`getBoundingClientRect()`/`getComputedStyle()` — Chrome DevTools MCP is the natural tool for this, being the debug/inspection-leaning one) against the Figma numbers, adjust, and re-measure until it converges — rather than translating the reference code once and treating a single later `visual-check` pass as the only checkpoint. **Chrome DevTools MCP uses a single browser profile with no isolation by default** — if another process/session already has it open, it silently refuses to start a second instance; fall back to the Playwright MCP session (the one `visual-check` itself uses) for the same measurement if that happens, rather than concluding the check can't be done.
...
- Don't treat a named breakpoint-variant sibling frame (e.g. a "Mobile (375px)" frame under the same section) as a derived reflow of the desktop frame — pull its own `get_design_context` (see Step 2) rather than guessing how the desktop values should scale down.
