...
- If a dev server (`npm run dev`) is available, do a real browser pass and compare against the `get_screenshot` output for a true pixel check — offer this explicitly rather than assuming the standalone render + hand-check is sufficient when the user needs pixel accuracy.
