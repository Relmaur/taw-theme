...
- **`figma-fidelity`** — "it doesn't look like the design" / "match Figma exactly" → the numbers-based fidelity pass: measure the live DOM against Figma's own sizes on both the desktop and mobile frames, close the gaps, and report drift. Encodes the recurring traps (unlayered `critical.scss` beating utilities, shared fluid spacing vs Figma's literal values, remapped `@theme` scales, Figma stretching photos, greedy text wrapping)
- **`figma-image-crop-fix`** — "the crop is off" → derives the exact pan/zoom (or stretch) treatment for an image from Figma's own generated CSS, and knows when a percentage transform is safe vs when to use `object-position`
