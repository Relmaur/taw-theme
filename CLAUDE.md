...
**Management Hub:** Opt-in via `wp-config.php` only (`define('TAW_HUB_ENABLED', true)`) — **not** theme code, no `inc/customizations.php` line. An authenticated `wp-json/taw-hub/v1/` receiver (`TAW\Hub`) for a central management Hub: telemetry, Vite asset deploy, block-config sync, cache flush. HMAC/Ed25519-signed requests; remote actions are a fixed allow-list, not an open command channel. Inert on every site without the constant (`Theme::boot()` subsystem #12 early-returns). Full detail: `taw/core` README § "Management Hub Integration".
